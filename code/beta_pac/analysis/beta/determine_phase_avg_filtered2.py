...
def main(path, subpath, mode, beta_type, axes, axes2, dir_type, dir_thresh, ideal_ref_slope = None):
    ...
    print(mode, beta_type, "%2.2f | %2.2f" % (np.float32(loc_burst_data.shape[0]/pre[0]), np.float32(loc_non_burst_data.shape[0]/pre[1]),))
    ...
    print("scores", np.sum(np.abs(result1.best_fit - loc_burst_data_m)), np.sum(np.abs(result2.best_fit - loc_non_burst_data_m)))
...
(data1, data2, loc_burst_fit)   = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "beta", "hf beta",
                                       axes[0, :], axes2[0, :], dir_type, dir_thresh)
(data3, data4, _)               = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "beta", "hf non beta",
                                       axes[1, :], axes2[1, :], dir_type, dir_thresh, loc_burst_fit)

dir_type = -1
dir_thresh = 0.1
(data5, data6, _)                = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "beta", "hf beta",
                                       axes3[0, :], axes4[0, :], dir_type, dir_thresh, loc_burst_fit)
(data7, data8, _)                = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "beta", "hf non beta",
...
#np.mean(np.asarray(data8, dtype = np.float32)[:, 0])
