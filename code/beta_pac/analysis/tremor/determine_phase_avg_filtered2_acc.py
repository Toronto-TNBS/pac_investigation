...
def get_values(path, subpath, mode, tremor_type):
    ...
    directionality = np.load("/mnt/data/Professional/UHN/pac_investigation/code/beta_pac/analysis/tremor/test.npy")
...
data7 = np.asarray(data6, dtype = np.float32);
...
stats = glmm.run(stat_data_12, ["score", "patient", "trial", "type"], ["continuous", "categorical", "continuous", "categorical"],
                  "score ~ type + (1|patient)", "list(score = contr.sum, patient = contr.sum, trial = contr.sum, type = contr.sum)",
                 "gaussian")
...
stats = glmm.run(stat_data_13, ["score", "patient", "trial", "type"], ["continuous", "categorical", "continuous", "categorical"],
                 "score ~ type + (1|patient)", "list(score = contr.sum, patient = contr.sum, trial = contr.sum, type = contr.sum)",
                 "gaussian")
stats = np.asarray(stats)
print(stats)
print(float(stats[2, 0])*3, "%05.03f, %05.03f, %05.03f" % ((float(stats[3, 0]) + float(stats[3, 1]))/float(stats[3, 1]), (float(stats[3, 0]) - float(stats[4, 0]) + float(stats[3, 1]))/float(stats[3, 1]), (float(stats[3, 0]) + float(stats[4, 0]) + float(stats[3, 1]))/float(stats[3, 1])))
stats = glmm.run(stat_data_14, ["score", "patient", "trial", "type"], ["continuous", "categorical", "continuous", "categorical"],
                  "score ~ type + (1|patient)", "list(score = contr.sum, patient = contr.sum, trial = contr.sum, type = contr.sum)",
                 "gaussian")
stats = np.asarray(stats)
print(stats)
