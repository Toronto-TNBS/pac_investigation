...
def main3():
    ...
    lf_tremor_burst_strength_idx = pre_labels.index("tremor burst lfp strength 1")
    lf_tremor_non_burst_strength_idx = pre_labels.index("tremor non burst lfp strength 1")
    tremor_burst_pac_strength_idx = pre_labels.index("pac burst strength 2")
    tremor_non_burst_pac_strength_idx = pre_labels.index("pac non burst strength 2")
    ...
    idx_list_burst_1 = np.asarray([lf_tremor_burst_strength_idx, patient_id_idx, trial_idx])
    idx_list_non_burst_1 = np.asarray([lf_tremor_non_burst_strength_idx, patient_id_idx, trial_idx])
    idx_list_burst_2 = np.asarray([tremor_burst_pac_strength_idx, patient_id_idx, trial_idx])
    idx_list_non_burst_2 = np.asarray([tremor_non_burst_pac_strength_idx, patient_id_idx, trial_idx])
    ...
    for data_idx in range(len(data)):
        stats = glmm.run(data[data_idx], labels, factor_type, formula, contrasts, data_type)
        print(np.asarray(stats), float(np.asarray(stats)[2, 0]))
        stats = np.asarray(stats)
        feat_idx = 0; print(float(stats[2, 0])*2, "%05.03f, %05.03f, %05.03f" % ((float(stats[3, feat_idx]) + float(stats[3, -1]))/float(stats[3, -1]), (float(stats[3, feat_idx]) - float(stats[4, feat_idx]) + float(stats[3, -1]))/float(stats[3, -1]), (float(stats[3, feat_idx]) + float(stats[4, feat_idx]) + float(stats[3, -1]))/float(stats[3, -1])))
