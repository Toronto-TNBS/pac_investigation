...
def get_values(path, subpath, mode, type, dir):
    ...
    for (f_idx, f_name) in enumerate(meta_info["file"]):
        ...
        if (f_name not in dir_names):
            continue
        curr_dir = np.sign(float(directionality[dir_names.index(f_name), 1]))
        if (curr_dir != dir):
            continue
        ...
        sin_fit_list.append([data[burst_spike_offset]])
...
def main(path, subpath, mode, type, axes, dir):
    (data, fit, patient_ids, trial_ids, pac_scores) = get_values(path, subpath, mode, type, dir)
...
cond_data_pre = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "tremor", "hf tremor", [axes[0]], +1)
cond_data_succ = main("/mnt/data/Professional/UHN/projects/old/pac_investigation/results/", "/data/2/", "tremor", "hf tremor", [axes[1]], -1)

data1 = np.asarray(cond_data_pre[0, :, :], dtype = np.float32).transpose();
data2 = np.asarray(cond_data_succ[0, :, :], dtype = np.float32).transpose();
...
for loc_stat_data in stat_data:
    ...
    #print(stats)
    print(float(stats[2, 0])*len(stat_data), "%05.03f, %05.03f, %05.03f" % ((float(stats[3, 0]) + float(stats[3, 1]))/float(stats[3, 1]), (float(stats[3, 0]) - float(stats[4, 0]) + float(stats[3, 1]))/float(stats[3, 1]), (float(stats[3, 0]) + float(stats[4, 0]) + float(stats[3, 1]))/float(stats[3, 1])), float(stats[3, 0]) + float(stats[3, 1]), float(stats[3, 1]))
