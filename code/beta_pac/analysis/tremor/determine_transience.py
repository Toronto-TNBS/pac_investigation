...
def get_values(path, subpath, mode):
    ...
    for (f_idx, f_name) in enumerate(meta_info["file"]):
        ...
        loc_lf_data = [values[0], meta_info["tremor lfp strength 1"][f_idx], patient_idx, trial_idx]
        loc_hf_b_data = [values[1], meta_info["tremor burst strength 1"][f_idx], patient_idx, trial_idx]
        loc_hf_nb_data = [values[2], meta_info["tremor non burst strength 1"][f_idx], patient_idx, trial_idx]
        loc_dmi_b_data = [values[3], meta_info["tremor burst strength 1"][f_idx], patient_idx, trial_idx]
        loc_dmi_nb_data = [values[4], meta_info["tremor non burst strength 1"][f_idx], patient_idx, trial_idx]
...
main("/mnt/data/Professional/UHN/pac_investigation/results/", "/data/3/", "tremor")
