...
def main(write, annotate):
    ...
    if (write):
        meta_file.modify_sheet_from_array("tremor", meta_data, meta_labels)
        meta_file.write_file()
    
    visualization(lfp_data, hf_default_data, txt_data, annotate)
...
def get_data(path = "../../../../data/meta.ods", sheet = "tremor"):
    ...
    lfp_tremor_idx = pre_labels.index("tremor lfp strength 1")
    hf_default_tremor_idx = pre_labels.index("tremor overall strength 1")
    hf_burst_tremor_idx = pre_labels.index("tremor burst strength 1")
    hf_non_burst_tremor_idx = pre_labels.index("tremor non burst strength 1")
    
    lf_3_index = pre_labels.index("lf manual")
    hf_3_index = pre_labels.index("hf manual")
    
    lfp_data = np.asarray(pre_data[np.argwhere(np.asarray(pre_data[:, valid_idx], dtype = np.int) == 1), [lfp_tremor_idx, lf_3_index]], dtype = np.float32)
    hf_default_data = np.asarray(pre_data[np.argwhere(np.asarray(pre_data[:, valid_idx], dtype = np.int) == 1), [hf_default_tremor_idx, hf_3_index]], dtype = np.float32)
    hf_burst_data = np.asarray(pre_data[np.argwhere(np.asarray(pre_data[:, valid_idx], dtype = np.int) == 1), [hf_burst_tremor_idx, hf_3_index]], dtype = np.float32)
    hf_non_burst_data = np.asarray(pre_data[np.argwhere(np.asarray(pre_data[:, valid_idx], dtype = np.int) == 1), [hf_non_burst_tremor_idx, hf_3_index]], dtype = np.float32)
